...
class MessageAdapter:
    def __init__(self, save_dir: str, voice_to_text: bool = True):
    ...
    def _adapt_image(
        ...
        extra = self._download_media(msg)
        return self._finish(
            msg,
            chat_name,
            ContentType.IMAGE,
    ...
    def _adapt_file(
        ...
        logger.debug("类型分支: file, 来源: msg.type")
        ...
        try:
            path = msg.download(dir_path=self._save_dir)
            extra["path"] = str(path)
            extra["download_status"] = "success"
            logger.debug("文件下载成功: %s", path)
    ...
    def _download_media(self, msg: Any) -> Dict[str, Any]:
        """下载图片/视频媒体文件。"""
        extra: Dict[str, Any] = {}
        try:
            path = msg.download(dir_path=self._save_dir)
    ...
    def _build_dto(
        ...
        fingerprint = self._generate_fingerprint(
            chat_name, sender, content_type.value, content, extra
        )
    ...
    def _generate_fingerprint(
        ...
        raw = f"{chat_name}\x00{sender}\x00{content_type}\x00{content_preview}\x00{extra_key}"
